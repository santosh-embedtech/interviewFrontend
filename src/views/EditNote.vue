...
function saveNote() {
  ...
  axios
    .patch(`/note/${route.params.id}`, form)
    .then(({ data }) => {
...
<template>
  <div class="flex flex-col space-y-4 justify-center items-center h-screen">
    <h1 class="text-2xl font-bold">Update Note</h1>
    <div class="w-1/3 rounded-md border-gray-200 p-4 border">
      <Form @submit="saveNote" v-slot="{ errors }" class="w-full md">
        ...
        <div>
          <label for="address" class="block mb-2 text-sm font-medium text-gray-900">Note</label>
          <Field
            v-model="form.note"
            v-slot="{ field }"
            id="note"
